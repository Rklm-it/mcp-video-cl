...
- **Голос:** `edge` — бесплатные нейроголоса Microsoft (`ru-RU-DmitryNeural`, `ru-RU-SvetlanaNeural`);
  `elevenlabs` — платно, лучше звучит, нужен API-ключ из подписки ElevenLabs.
- **Картинки:** `pollinations` — бесплатно; `gemini` — Nano Banana через Gemini API (платно);
  `openai` — любой OpenAI-совместимый API картинок. Вместо генерации в сцене можно указать `media`:
  ссылку или файл из `data/videos` — например, клип, сделанный в Veo или Kling.
