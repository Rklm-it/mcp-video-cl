...
## Фабрика рилсов (по желанию)

Тот же сервер умеет делать вертикальные ролики и выкладывать их. Сценарии пишет и проверяет
Claude, а сервер делает то, что требует денег или ресурсов: озвучку, картинки, сборку и загрузку.
Включается строкой `REELS_ENABLED=1` в `.env` (остальные настройки — в `.env.example`, раздел «Reels»).

| Инструмент | Что делает |
|---|---|
| `create_reel` | Сценарий из 4–7 сцен → ролик 1080×1920: озвучка, картинки с плавным движением, крупные субтитры, для рекламных роликов — маркировка «Реклама. … erid» и плашка оффера. Возвращает кадры для проверки. |
| `edit_reel` | Поменять текст или картинку отдельных сцен и пересобрать (остальные картинки не генерируются заново). |
| `save_offer` | Сохранить рекламный оффер: рекламодатель, erid, текст плашки, партнёрская ссылка. |
| `publish_reel` | Выложить в Telegram-канал и YouTube Shorts. |
| `list_reels`, `get_reel`, `reject_reel` | Список, просмотр, отклонение. |
| `reels_setup` | Что настроено: голос, картинки, куда публикуются ролики, офферы. |
| `scout_channel` | Самые просматриваемые ролики канала-конкурента, чтобы разобрать их через `analyze_video`. |

Что за чем:

- **Голос:** `edge` — бесплатные нейроголоса Microsoft (`ru-RU-DmitryNeural`, `ru-RU-SvetlanaNeural`);
  `openai` — любой OpenAI-совместимый `/audio/speech`, например GPT-4o mini TTS через Timeweb AI Gateway;
  `elevenlabs` — платно, звучит лучше всех, нужен API-ключ из подписки ElevenLabs.
- **Картинки:** `pollinations` — бесплатно; `openai` — любой OpenAI-совместимый `/images/generations`
  (Timeweb AI Gateway: Gemini Image, GPT Image, FLUX.2); `gemini` — Nano Banana через Gemini API
  или посредника с тем же API. Вместо генерации в сцене можно указать `media`: ссылку или файл
  из `data/videos` — например, клип, сделанный в Veo или Kling.
- **Живое видео:** картинка сцены становится первым кадром ролика Veo 3.1 (`REELS_VIDEO=veo`, платно
  за секунду). С `REELS_ANIMATE_ALL=1` видео — каждая сцена, клипы генерируются параллельно, а ролик
  собирается в фоне и сам приходит в Telegram на проверку. Лимит на ролик — `REELS_MAX_ANIMATED`.
  Если генерация сцены не удалась, она остаётся картинкой, а ролик всё равно собирается.
- **Музыка:** положите разрешённые треки (например, из фонотеки YouTube) в папку `REELS_MUSIC_DIR` —
  под голос подмешивается случайный трек, тихо и с затуханием в конце.
- **Сборка:** ffmpeg, бесплатно, на сервере.
- **Проверка перед публикацией:** с `after="review"` готовый ролик приходит вам в Telegram с кнопками
  «Опубликовать» / «Отклонить». Или смотрите кадры прямо в чате с Claude и публикуйте через `publish_reel`.
- **VK Клипы** пока выкладываются вручную.

Готовый вариант с оплатой в рублях — **Timeweb AI Gateway**: один ключ на голос и картинки.
Настройки — в `.env.example` (пресет «Timeweb AI Gateway»); название модели картинок скопируйте
из панели Gateway.

Конвейер на каждый день: в Claude Code заведите расписание (routine) с заданием вроде
«посмотри лучшие ролики конкурентов через scout_channel, напиши 3 сценария про деньги по моему шаблону,
собери их через create_reel с after=review». Утром в Telegram будут ждать три черновика.

YouTube подключается один раз:
